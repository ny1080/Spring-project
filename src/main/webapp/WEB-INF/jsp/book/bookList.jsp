								...
								<div class="left" style="width: 50%;" align="right">
									<div style="float: left; width: 40%;">
										<select class="selectpicker form-control"
											...
											id="group1" name="group1"><option selected>전체
									...
									<div style="float: right; width: 60%;">
										<select class="selectpicker form-control"
											style="font-size: 14px; margin-top: 10px; vertical-align: baseline;"
											id="group2" name="group2"><option selected>전체
											...
											<tr>
												<td class="hidden-xs hidden-sm hidden-md genre" align="left"><c:choose>
														<c:when test="${empty book.imageurl }">
															<a href="#"
																onclick="window.open('/book/${book.bookCode}',
												...
												<td class="hidden-xs genre" align="left">${book.group1 }</td>
												<c:choose>
													<c:when test="${book.rentStatus=='0'}">
														<td align="left"><mark>대출가능</mark></td>
														<td align="left"><button
																class="btn btn-default borrowRequest" type="button"
																id="borrowbook" value="${book.bookCode}">대출</button></td>
													</c:when>
													<c:when
														test="${book.rentStatus=='1' and book.rsvStatus=='1'}">
														<td>예약중</td>
														<td></td>
													</c:when>
													<c:when
														test="${book.rentStatus=='1' and book.rsvStatus=='0'}">
														<td><p style="color: blue;">대출요청중</p></td>
														<td>
															<!-- 														<button class="btn btn-default" type="button" -->
															<!-- 																id="reservebook" --> <%-- 																onClick="location.href='/reservation?bookCode=${book.bookCode}'">예약</button> --%>
														</td>
													</c:when>
													<c:when
														test="${book.rentStatus=='2' and book.rsvStatus=='1'}">
														<td>예약중</td>
														<td></td>
													</c:when>
													<c:when
														test="${book.rentStatus=='2' and book.rsvStatus=='0'}">
														<td><p style="color: blue;">대출중</p> 반납일
															:${book.returndate}</td>
														<td></td>
													</c:when>
													<c:when test="${book.rentStatus=='4'}">
														<td>대출 정지</td>
														<td></td>
													</c:when>
													<c:when test="${book.rentStatus=='5'}">
														<td>예약중</td>
														<td></td>
													</c:when>
													<c:otherwise>
